...
public:		// User declarations
		...
		AnsiString numFile, path, extFile, fnamePrev, fnameNext;
        int countDigitsFname;
		int* formats;
		bool newFile;
        bool use_fltr_post;  //при открытии нового файла использовать текущие значения фильтров и постобработок
		__fastcall TForm1(TComponent* Owner);
        void SetListNumbersFiles(AnsiString, AnsiString);
        void __fastcall OpenImage(AnsiString fname);
		void __fastcall OpenBMP(AnsiString fname);
		void __fastcall DoChangeMem0ToMem1();
		void __fastcall AdapterBmpToMem0(bool, int H0, int W0, int* iH, int* iW);
