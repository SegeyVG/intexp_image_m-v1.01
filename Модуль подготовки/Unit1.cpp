...
bool CheckFileFormat(const AnsiString& rdir_i, const AnsiString& sname, const AnsiString& format, const AnsiString& ext) {
    // Регулярное выражение для извлечения числовой части из формата
//	std::regex re("%0*(\\d+)d|%\\s*(\\d+)d|%d");
//    std::smatch match;

//    // Проверяем формат на наличие числовой части
//    if (!std::regex_search(format.c_str(), match, re)) {
//        ShowMessage("Ошибка: неверный формат.");
//        return;
//    }
	bool ret = true;
